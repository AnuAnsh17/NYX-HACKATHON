            ...
            <div className="flex items-center gap-2">
              <span
                className={`w-1.5 h-1.5 rounded-full ${
                  connected ? "bg-nyx-valid pulse-valid" : "bg-nyx-broken pulse-broken"
                }`}
              />
              <span
          ...
          >
            {sorted.length === 0 ? (
              ...
              sorted.map((block) => (
                <div
                  key={block.index}
                  className={`flex items-center gap-4 px-3 py-2 rounded-sm transition-colors duration-300 ${
                    flashIndex === block.index
                      ? "bg-nyx-valid/10"
                      : "hover:bg-nyx-surface/50"
                  }`}
                >
                  <span className="mono-label text-nyx-dim w-48 flex-shrink-0 truncate">
                    {block.timestamp}
                  </span>
                  <span className="mono-label text-nyx-accent w-20 flex-shrink-0">
                    BLK-{block.index}
                  </span>
                  <span className="font-mono text-xs text-nyx-text flex-1 truncate">
                    {block.data}
                  </span>
                  <span className="mono-label text-nyx-dim flex-shrink-0">
